#!/usr/bin/env python3
"""
Complete Figure 3 Pipeline: Data Processing + Visualization
...
3. Creates the 9-panel showing all species and conditions
...
CONFIGS = {
    ...
    'Agelenopsis': {
        ...
        'period_data_file': "Ag_spider_analysis_comprehensive_with_LD_split.csv",
...
def plot_activity_panel(ax, data, condition, species_name, show_ylabel=True, show_xlabel=True):
    ...
    # X-axis label (shown on every row)
    if show_xlabel:
        if condition == 'LD':
            ax.set_xlabel('Zeitgeber Time (ZT)', fontsize=10)
        else:
            ax.set_xlabel('Subjective Hours (CT)', fontsize=10)
    else:
        # Don't remove tick labels, just don't add xlabel
        pass


def create_figure(output_files, output_path):
    """
    ...
    # Abbreviated italic species labels for column headers
    species_labels = {
        'Larinioides': 'L. cornutus',
        'Agelenopsis': 'A. pennsylvanica',
        'Steatoda': 'S. grossa'
    }
    col_letters = ['A', 'B', 'C']
    ...
    for row_idx, condition in enumerate(condition_order):
        for col_idx, species in enumerate(species_order):
            ...
            show_xlabel = True  # Show x-axis label on every row
            ...
            # Top row: add italic species name header and letter label
            if row_idx == 0:
                ax.text(0.5, 1.30, species_labels[species],
                        transform=ax.transAxes,
                        ha='center', va='bottom',
                        fontsize=14, fontstyle='italic',
                        clip_on=False)
                ax.text(0.0, 1.17, col_letters[col_idx],
                        transform=ax.transAxes,
                        ha='left', va='bottom',
    ...
    # Adjust layout
    plt.tight_layout()
    plt.subplots_adjust(left=0.08, right=0.98, top=0.88, bottom=0.07,
                       wspace=0.11, hspace=0.35)
...
def main():
    print("=" * 70)
    print("FIGURE 3 COMPLETE PIPELINE")
    ...
    # Create Figure 2
    create_figure(output_files, OUTPUT_FILENAME)

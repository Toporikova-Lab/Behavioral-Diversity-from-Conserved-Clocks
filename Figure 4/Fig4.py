...
def draw_activity_grid(axes, results):
    ...
    for col, name in enumerate(species_names):
        for row, cond in enumerate(CONDITIONS):
            ...
            if col == 0:
                # Condition label rotated to the left of the panel
                ax.text(-0.52, 0.5, cond, transform=ax.transAxes,
...
def build_figure(results, light_vals, mask_vals, grid):
    ...
    # Outer GridSpec: left column (A + B) vs right 3x3 (C)
    gs_outer = GridSpec(1, 2, figure=fig,
                        width_ratios=[1.8, 3.0],
                        left=0.01, right=0.97, top=0.98, bottom=0.08,
                        wspace=0.30)

    # Left: schematic (larger) stacked above heatmap (square)
    gs_left = GridSpecFromSubplotSpec(2, 1, subplot_spec=gs_outer[0, 0],
                                     height_ratios=[2.0, 2.5], hspace=0.38)

    # Right: tightly packed 3x3 activity grid
    gs_right = GridSpecFromSubplotSpec(3, 3, subplot_spec=gs_outer[0, 1],
                                      hspace=0.38, wspace=0.20)

    ax_sch = fig.add_subplot(gs_left[0])
    ax_heat = fig.add_subplot(gs_left[1])
    act_axes = [[fig.add_subplot(gs_right[r, c]) for c in range(3)]
                for r in range(3)]
    ...
    # Scale schematic 30% larger, preserving its current aspect ratio.
    # Anchor the bottom edge relative to the heatmap; expand up and right.
    fig.canvas.draw()
    sp = ax_sch.get_position()
    hp = ax_heat.get_position()
    gap = sp.y0 - hp.y1          # gap between schematic bottom and heatmap top
    ax_sch.set_position([
        sp.x0,
        hp.y1 + gap,             # keep same gap above heatmap
        sp.width  * 1.3,
        sp.height * 1.3,
    ])

    # Place panel labels with exact alignment:
    # A shares x with B (same left column), y with C (same top row)
    fig.canvas.draw()  # re-finalise after resize
    hp = ax_heat.get_position()
    cp = act_axes[0][0].get_position()

    b_x = hp.x0 - 0.24 * hp.width
    b_y = hp.y1 + 0.02 * hp.height
    c_x = cp.x0 - 0.50 * cp.width
    c_y = cp.y1 + 0.18 * cp.height

    fig.text(b_x, c_y, 'A', fontsize=15, fontweight='bold',
             ha='left', va='bottom', clip_on=False)
    fig.text(b_x, b_y, 'B', fontsize=15, fontweight='bold',
             ha='left', va='bottom', clip_on=False)
    fig.text(c_x, c_y, 'C', fontsize=15, fontweight='bold',
             ha='left', va='bottom', clip_on=False)
...
def main():
    ...
    fname = FILE_OUTPUT
    fig.savefig(fname, dpi=DPI_OUTPUT, format = 'png', bbox_inches='tight')
    print(f"Saved: {fname}")
